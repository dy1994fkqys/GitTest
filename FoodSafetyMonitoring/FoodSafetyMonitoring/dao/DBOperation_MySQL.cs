...
namespace FoodSafetyMonitoring.dao
{
    class DBOperation_MySQL : IDBOperation
    {
        ...
        public UserInfo GetMenu(string loginName, string password)
        {
            ...
            string sql = string.Format("select reco_pkid uid, INFO_USER uname, fk_dept deptid, SUB_NAME menu " +
                                       "from v_user_sub " +
            ...
            try
            {
                ...
                if (table.Rows.Count == 1)
                {
                    ...
                    userInfo.Menus.AddRange(table.Rows[0][3].ToString().Split(new char[] { ',' }));
                }
                return userInfo;
            }
            catch (Exception e)
